...
sql_query = f"""
    SELECT *
    FROM vinty.{SCHEMA}.inc_rebag__sold_products
"""
try:
    ...
    with col1a:
        vendor_selected = st.selectbox(
            ...
            index=st.session_state.vendors_list.index(st.session_state.selected_vendor),
        )
    ...
    st.title("Sales Per Day Per Vendor Line Graph")

    fig, ax = plt.subplots(figsize=(12, 6))

    vendor_sales = sales_per_day.groupby("vendor")["sales_count"].sum().reset_index()
    ...
    sns.lineplot(data=revenue_per_day, x="Date", y="Total Revenue", marker="o", ax=ax)
    ...
    st.title("Price vs Product Type Dot Plot")

    product_counts = filtered_df["product_type"].value_counts().reset_index()
    product_counts.columns = ["product_type", "count"]

    sorted_product_types = product_counts.sort_values("count", ascending=False)[
        "product_type"
    ]

    filtered_df["product_type"] = pd.Categorical(
        filtered_df["product_type"], categories=sorted_product_types, ordered=True
    )
    ...
    vendor_sales_sorted = vendor_sales.sort_values(by="products_sold", ascending=False)

    plt.figure(figsize=(10, 6))
    sns.barplot(x="products_sold", y="vendor", data=vendor_sales_sorted, palette="Set1")
    ...
    plt.figure(figsize=(10, 6))
    sns.stripplot(
        x="condition", y="price", data=filtered_df, jitter=True, size=5, palette="Set1"
    )
